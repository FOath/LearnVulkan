...
public:
	...
	// }ubo;
	//
	// This way we can just memcpy the ubo data to the ubo
	...
	// It define interface (without binding any actual data) between the shader stages used by the pipeline and the shader resources
	// A pipeline layout can be shared among multiple pipeline as long as their interfaces match
	VkPipelineLayout pipelineLayout;

	// Pipelines (often called "pipeline state objects") are used to bake all states that affect a pipeline
	// While in OpenGL every state can be chagned at (almost) any time, Vulkan requires to layout the graphics (and compute) pipeline states upfront
	...
	// Semephores are used to coordinate operations within the graphics queue and ensure correct command ordering
	...
	~VulkanExample()
	{
		...
		// Note: Inherited desctructor cleans up resources stored in base class
	...
	void createSynchronizationPrimitives()
	{
		for (uint32_t i = 0; i < MAX_CONCURRENT_FRAMES; ++i)
		{
			...
			// Semaphores used to ensure that all commands submitted have been finished before submitting the image to the queue
			VK_CHECK_RESULT(vkCreateSemaphore(device, &semaphoreCI, nullptr, &renderCompleteSemaphores[i]));

			// Fences (Used to check draw command buffer completion)
			VkFenceCreateInfo fenceCI{};
			fenceCI.sType = VK_STRUCTURE_TYPE_FENCE_CREATE_INFO;
			// Create in singaled state so we don't wait on first render of each command buffer
	...
	// Also uploads them to device local memory using staging and initializers vertex input and attribute binding to match the vertex buffer
	void createVertexBuffer()
	{
		// A note on memory management in Vulkan is general:
		// This is a verty complex topic and while it's fine for an example application to small individual memory allocations that is not 
		// what should be done a real-world application, where you should allocate larget chunks of memory at once instead.
		...
		// To keep this sample easy to follow, there is not check for that in place
	...
	void createDescriptorPool()
	{
		...
		// tpyeCounts[1].descriptorCount = 2;
	...
	{
		// Binding 0: Uniform buffer (Vertex buffer)
		...
		// Create the pipeline layout that is used to generate the rendering pipeline that are based on this descriptor set layout
	...
	// The descirptor sets make use of the descriptor set layouts created above
	...
	// Note: Override of virtual function in the base function and called from within VulkanExampleBase::prepare
	void setupDepthStencil()
	{
		...
		// Stencil aspect should only be set on depth + stencil formats (VK_FORMAT_D16_UNORM_S8_UINT..VK_FORMAT_D32_SLFOAT_S8_UINT)
	...
	// Render pass setup
	// Render passes are a new concept in Vulkan. They describe the attachments used during rendering and may contain multiple subpassed with attachment dependencies
	// Thsi allows the driver to know up-front what the rendering will look like and is a good opportunity to optimize especially on tile-based renderers (with multiple subpassed)
	...
	void setupRenderPass()
	{
		...
		attachments[0].finalLayout = VK_IMAGE_LAYOUT_PRESENT_SRC_KHR; // As we want to present the color buffer to the swapchain, we transition to PRESENT_KHR
		...
		depthReference.layout = VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL; // Attachemnt used as depth/stencil used during the subpass
		...
		renderPassCI.attachmentCount = static_cast<uint32_t>(attachments.size()); // Number of attachments used by this renderpass
	...
	// Shaders are compiled oofline from e.g. GLSL using the reference glsllang compiler
	...
	void createPipeline()
	{
		...
		// Construt the different state making up the pipeline
		...
		// This example does not make use of multi sampling (for anti-aliasing), the state must still be set and pass to the pipeline
		...
		// Specifies the vertex input parameter for a pipeline
		...
		// Color attributes is three 32 bit signed (SFLOAT) floats (R32 G32 B32)
	...
	{
		// Prepare and initializa the per-frame uniform buffer blocks containing shader uniforms
	...
	virtual void render()
	{
		...
		// Copy the current matrics to the current frame's uniform buffer
		...
		VK_CHECK_RESULT(vkBeginCommandBuffer(commandBuffers[currentBuffer], &cmdBuffInfo));
		...
		// Bind descriptor set for the current frame's uniform buffer, so the shader uses the data from that buffer for this view
		...
		submitInfo.pCommandBuffers = &commandBuffers[currentBuffer]; // Command buffer(s) to execute in this batch (submission)
		...
		// This ensures that the image is not presented to the windowing system untill all commands have been submitted
		...
		{
			throw "Could not present the image to the swap chian!";
		}
	}
};
