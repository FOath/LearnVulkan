...
#if (defined(VK_USE_PLATFORM_MACOS_MVK) && defined(VK_EXAMPLE_XODE_GENERATED))
...
VkResult VulkanExampleBase::createInstance(bool enableValidation)
{
	...
	// Get extension supported by the instance and store for later use
	uint32_t extCount = 0;
	...
	if (settings.validation)
	{
		...
		{
			instanceCreateInfo.ppEnabledExtensionNames = &validationLayerName;
	...
	// If the debug utils extension is present we set up debug functions, so sampels an label objects for debugging
...
{
	// Create one command buffer for each chain image and reuse for rendering
...
{
	// SRS - for non-apple platforms, handle benchmarking here within VulkanExampleBase::renderLoop()
...
void VulkanExampleBase::updateOverlay()
{
	...
	ImGui::Text("%.2f ms/frame (%.1d fps)", (1000.f / lastFPS), lastFPS);
...
bool VulkanExampleBase::initVulkan()
{
	...
	if (commandLineParser.isSet("gpuselection"))
	{
		...
		if (index > gpuCount - 1)
		{
			std::cerr << "Selected device index " << index << " is out of range, reverting to devic 0 (use -listgpus to show available Vulkan devices)" << "\n";
		}
		else
		{
	...
	// Store properties (including limits), features and memory properties of the physical device (so that example can check against them)
	...
	// and encapsulated function related to a device
	...
	// Create a semaphore used to synchroniza command submission
	// Ensures that the image is not presented until all commands have been submitted and executed
	VK_CHECK_RESULT(vkCreateSemaphore(device, &semaphoreCreateInfo, nullptr, &semaphores.renderComplete));

	// Set up submit info structure
	// Semaphore will stay the same during applicaiton lifetime
...
{
	// Wait fences to sync command buffer acces
