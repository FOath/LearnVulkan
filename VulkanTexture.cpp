...
namespace vks
{
	...
	/// </summary>
	/// <param name="filename">File to load (supports .ktx)</param>
	/// <param name="format">Vulkan format of the image data stored in the file</param>
	/// <param name="device">Vulkan device to create the texture on</param>
	/// <param name="copyQueue">Queue used for the texture staging copy commands (must support transfer)</param>
	/// <param name="imageUsageFlags">(Optional) imageUsageFlags Usage flags for the texture's image (default to VK_IMAGE_USAGE_SAMPLED_BIT)</param>
	/// <param name="imageLayout">(Optinal) imageLayout Usage layout for the texture (default VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL)</param>
	/// <param name="forceLinear">(Optinal) forceLinear Force linear tiling (not advised, defaults to false)</param>
	void Texture2D::loadFromFile(std::string filename, VkFormat format, vks::VulkanDevice* device, VkQueue copyQueue, VkImageUsageFlags imageUsageFlags, VkImageLayout imageLayout, bool forceLinear)
	{
		...
		// Use s separate command buffer for texture loading
		...
		else 
		{
			...
			// depending on implementation (e.g. no mip maps, only onel ayer, etc.)
	...
	/// <summary>
	/// Create a 2D texture from a buffer
	/// </summary>
	/// <param name="buffer">Buffer containg texture data to upload</param>
	/// <param name="bufferSize">Size of the buffer in machine units</param>
	/// <param name="format">vulkan format of the image data stored in the file</param>
	/// <param name="texWidth">width of the texture to create</param>
	/// <param name="texHeight">height of the texture to create</param>
	/// <param name="device">Vulkan device to create the texture on</param>
	/// <param name="copyQueue">Queue used for the texture staging copy commands (must support transfer)</param>
	/// <param name="filter">(Optional) filter Texture filtering for the sampler (defaults to VK_FILTER_LINEAR)</param>
	/// <param name="imageUsageFlags">(Optional) Usage flags for the texture's image (default to VK_IMAGE_USAGE_SAMPLED_BIT)</param>
	/// <param name="imageLayout">(Optional) Usage layout for the texture (defaults VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL)</param>
	...
	/// </summary>
	/// <param name="filename"> File to load (support .ktx)</param>
	/// <param name="format"> Vulkan format of the image data stored in the file </param>
	/// <param name="device"> Vulkan device to create the texture on </param>
	/// <param name="copyQueue"> Queue used for the texture staging copy commands (must support transfer)</param>
	/// <param name="imageUsageFlags"> (Optional) imageUsageFlags Usage flags for the texture's image (default to VK_IMAGE_USAGE_SAMPLED_BIT) </param>
	/// <param name="imageLayout"> (Optional) imageLayout Usage layout for the texture (default VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL) </param>
	void Texture2DArray::loadFromFile(std::string filename, VkFormat format, vks::VulkanDevice* device, VkQueue copyQueue, VkImageUsageFlags imageUsageFlags, VkImageLayout imageLayout)
	{
		...
		// Get memory requirement for the staging buffer (alignment, memory type bits)
		vkGetBufferMemoryRequirements(device->logicalDevice, stagingBuffer, &memReqs);
